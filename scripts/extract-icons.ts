...
if (fs.existsSync(iconsDir)) {
  const icons = fs.readdirSync(iconsDir).filter(dir =>
    fs.statSync(path.join(iconsDir, dir)).isDirectory()
  );
  ...
  for (const icon of icons) {
    ...
    if (fs.existsSync(regularPath)) {
      ...
      const pathsMatch = [...content.matchAll(/jsx\("path",\s*({[^}]+})\s*\)/g)];

      if (pathsMatch.length > 0) {
        output[icon] = pathsMatch.map(m => {
            try {
  ...
  fs.writeFileSync(path.join(rootDir, 'src/components/icons.json'), JSON.stringify(output, null, 2));
